...
todoRoutes.get('/', async (c) => {
  ...
  const tagRows: TagRow[] = todoIds.length > 0
    ? (await db.select().from(todoTags).where(inArray(todoTags.todoId, todoIds))) as TagRow[]
    : [];
...
todoRoutes.post('/', async (c) => {
  ...
  if (tagNames.length > 0) {
    await db.insert(todoTags).values(
      tagNames.map((name) => ({ todoId: createdTodo.id, name }))
    );
  }
...
todoRoutes.get('/:id', async (c) => {
  ...
  const tagRows = (await db.select().from(todoTags).where(eq(todoTags.todoId, todo.id))) as TagRow[];
  return c.json({
    ...todo,
...
todoRoutes.patch('/:id', async (c) => {
  ...
  if (body.dueDate !== undefined) updateFields.dueDate = body.dueDate ? new Date(String(body.dueDate)) : null;
  if (body.priority !== undefined) updateFields.priority = ['Low', 'Medium', 'High'].includes(String(body.priority)) ? String(body.priority) : 'Low';
  if (body.completed !== undefined) updateFields.completed = Boolean(body.completed);
  ...
  if (Array.isArray(body.tags)) {
    ...
    const tagNames: string[] = body.tags.map((tag: unknown) => String(tag).trim()).filter(Boolean);
    if (tagNames.length > 0) {
      await db.insert(todoTags).values(tagNames.map((name) => ({ todoId: id, name })));
    }
  }

  const currentTags: TagRow[] = await db.select().from(todoTags).where(eq(todoTags.todoId, id)) as TagRow[];
  return c.json({
    todo: {
      ...updatedTodo,
